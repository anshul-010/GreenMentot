...
export const Table = () => {
    let data = [
        ...
        {month:"feb",emissions:2205,ratio:99.8,change:"3%↓"},
        {month:"mar",emissions:2345,ratio:122.6,change:"12%↓"},
        {month:"apr",emissions:1503,ratio:130.0,change:"16↓"},
        {month:"may",emissions:4634,ratio:124.2,change:"-8%↑"},
        {month:"jun",emissions:4634,ratio:124.2,change:"-13%↑"},
        {month:"jul",emissions:3487,ratio:165.1,change:"26%↓"},
    ]
  return (
    <div id='table-container'>
        <div className='top-head'>
            <div className='wise'>
                <p>Supplier-wise</p>
                <svg xmlns="http://www.w3.org/2000/svg" color="rgba(0, 0, 0, 0.80);" width="21" height="19" viewBox="0 0 21 19" fill="none">
        ...
        <table id='table' >
            <thead>
                <tr className='theading'>
                    ...
                    <th className='th1'>EMISSIONS(IN CO2E)</th>
                    <th className='th1'>REVENUE/EMISSION RATIO</th>
                    <th className='th1'>YOY R/E CHANGE</th>
                </tr>
            </thead>
            <tbody>
                {data?.map((ele)=>{
                    const numericValue = parseFloat(ele.change.match(/-?\d+/)[0]);
                    return <tr>
                        <td>{ele.month}</td>
                        <td>{ele.emissions}</td>
                        <td>{ele.ratio}</td>
                        <td style={{ color: numericValue < 0 ? 'red' : 'green' }}>{ele.change}</td>
